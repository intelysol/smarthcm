...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-5">
        ...
        <div class="mt-4 sm:mt-0 flex items-center space-x-3">
            ...
            <button onclick="document.getElementById('create-requisition-modal').classList.remove('hidden')" class="inline-flex items-center px-3.5 py-2 rounded-lg text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm transition">
...
<div id="create-requisition-modal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white border border-slate-200 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 class="text-lg font-bold text-slate-900">Create Job Requisition</h3>
            <button onclick="document.getElementById('create-requisition-modal').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 p-1">&times;</button>
        </div>
        <form onsubmit="handleCreateRequisition(event)" class="space-y-4">
            <div>
                <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Position / Job Title</label>
                <input type="text" required placeholder="Senior Distributed Systems Engineer" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-indigo-500">
            </div>
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Headcount Slots</label>
                    <input type="number" required value="1" min="1" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-indigo-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Target Hire Date</label>
                <input type="date" required value="{{ now()->addDays(45)->toDateString() }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-indigo-500">
            </div>
            <div class="pt-2 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('create-requisition-modal').classList.add('hidden')" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg transition">Cancel</button>
                <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition shadow-sm">Save & Open Vacancy</button>
            </div>
        </form>
    </div>
</div>
...
function handleCreateRequisition(e) {
    e.preventDefault();
    document.getElementById('create-requisition-modal').classList.add('hidden');
    window.showNotification('success', 'Job requisition registered. Published to talent pipeline and internal vacancy board.');
}
</script>
@endsection
