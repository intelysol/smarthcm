...
Route::prefix('payroll')->middleware(['web'])->group(function (): void {
    Route::get('/', function (\Illuminate\Http\Request $request) {
        if (auth()->check()) {
            return app(\App\Domains\Payroll\Http\Controllers\PayrollUIController::class)->dashboard($request);
        }
        return app(\App\Domains\PublicWebsite\Http\Controllers\PublicWebsiteController::class)->feature($request, 'payroll');
    })->name('payroll.public');

    Route::middleware(['auth'])->group(function (): void {
        Route::get('/dashboard', [PayrollUIController::class, 'dashboard'])->name('payroll.dashboard');
